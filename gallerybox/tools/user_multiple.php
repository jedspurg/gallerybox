...
if ($valt->validate('upload')) {
	if (isset($_FILES['Filedata']) && (is_uploaded_file($_FILES['Filedata']['tmp_name']))) {
		...
		if (!($resp instanceof FileVersion)) {
			...
			$fu = new User();
			$fuID = $fu->getUserID();
			$setName = 'user_gallery_'.$fuID;
			
			$fs = FileSet::getByName($setName);
			
			if(!is_object($fs)){
				$fs = FileSet::createAndGetSet($setName, FileSet::TYPE_PUBLIC, $fuID);
			}
			
			$globalSet = FileSet::getByName('GalleryBoxCollection');

			
			$fsf = $fs->addFileToSet($respf);
			$fsf = $globalSet->addFileToSet($respf);
