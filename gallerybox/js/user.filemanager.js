...
ccm_alActivateMenu = function(obj, e) {
	...
	*/
	...
	if (!bobj) {
		...
		el.className = "ccm-menu gbx-menu";
		...
		if ($(obj).attr('ccm-file-manager-can-view') == '1') {
			html += '<li><a class="ccm-icon"  href="' + CCM_REL + 'gallerybox/image/' + fID + '"><span style="background-image: url(' + CCM_IMAGE_PATH + '/icons/design_small.png)">'+ ccmi18n_filemanager.view + '<\/span><\/a><\/li>';
		}
		if ($(obj).attr('ccm-file-manager-can-download') == '1') {
			html += '<li><a class="ccm-icon" id="menuDownload' + fID + '" target="' + ccm_alProcessorTarget + '" href="' + CCM_TOOLS_PATH + '/files/download?fID=' + fID + '"><span style="background-image: url(' + CCM_IMAGE_PATH + '/icons/add.png)">'+ ccmi18n_filemanager.download + '<\/span><\/a><\/li>';	
		}
