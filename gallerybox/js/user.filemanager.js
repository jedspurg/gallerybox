...
ccm_triggerSelectFile = function(fID, af) {
	...
	obj.load(CCM_TOOLS_PATH + '/files/selector_data?fID=' + fID + '&ccm_file_selected_field=' + af, function() {
		...
		*/
		...
		obj.attr('ccm-file-manager-can-download', obj.children('div').attr('ccm-file-manager-can-download'));
		...
		switch(action) {
			...
			case "sets":
				jQuery.fn.dialog.open({
					...
					height: 400,
					modal: false,
					href: GBX_SET_TOOL + '?' + fIDstring + '&searchInstance=' + searchInstance,
					title: ccmi18n_filemanager.sets				
				});
				break;
			case "properties": 
	...
	$("#ccm-" + searchInstance + "-update-uploaded-details-form").ajaxSubmit(function(r1) {
		...
		if (form.length > 0) {
			form.ajaxSubmit(function(resp) {
				$("#ccm-" + searchInstance + "-sets-search-wrapper").load(GBX_SET_RELOAD, {'searchInstance': searchInstance}, function() {
					jQuery.fn.dialog.hideLoader();
					jQuery.fn.dialog.closeTop();
					ccm_parseAdvancedSearchResponse(resp, searchInstance);
	...
	*/
	...
	if (!bobj) {
		...
		if ($(obj).attr('ccm-file-manager-can-view') == '1') {
			html += '<li><a class="ccm-menu-icon ccm-icon-view" dialog-modal="false" dialog-append-buttons="true" dialog-width="90%" dialog-height="75%" dialog-title="' + ccmi18n_filemanager.view + '" id="menuView' + fID + '" href="javascript:void(0)" onclick="window.location=\'' + CCM_REL + '/index.php/gallerybox/image/' + fID + '\'">'+ ccmi18n_filemanager.view + '<\/a><\/li>';
		} 
		if ($(obj).attr('ccm-file-manager-can-download') == '1') {
		...
		html += '<li><a class="ccm-menu-icon ccm-icon-sets dialog-launch" dialog-modal="false" dialog-width="500" dialog-height="400" dialog-title="' + ccmi18n_filemanager.sets + '" id="menuFileSets' + fID + '" href="' + GBX_SET_TOOL + '?searchInstance=' + searchInstance + '&fID=' + fID + '">'+ ccmi18n_filemanager.sets + '<\/a><\/li>';
