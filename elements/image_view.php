...
<div id="gbx-sidebar" class="span3">
    	...
    	?>
        <h5><small><?php   echo t('by')?> </small><?php    echo $username?></h5>
            <div class="clearfix">
		...
		<h5><?php    echo t('Recent images')?><span class="icon book"></span></h5>
                     ...
                     <?php   if(count($galleryImages) < 6){
                      ...
                      <div><a  href="javascript:void(0);"><img src="<?php   echo $pkt->getPackageURL($pkg);?>/images/grid_image.jpg"/></a></div>       
                     	 <?php   }?>
                      <?php   }?>
                       <a href="<?php    echo View::url('/gallerybox/user',$fui->getUserID())?>" class="btn"><?php   echo t('View more')?><span class="icon rightarrow"></span></a>
                   </div> 
        ...
        <?php   if(Config::get(GBX_ALLOW_RSS)==1){?>
         <div id="gbx-rss">
            <div class="clearfix">
            <a href="<?php    echo View::url('/gbxrss',$fui->getUserID())?>" class="btn" target="_blank"><?php   echo t('Subscribe')?><span class="icon rss"></span></a>
    ...
    <div id="gbx-content-pane" class="span<?php   echo Config::get(GBX_GALLERY_INNER_CLASS)?>">
		<div id="gbx-image-view">
			...
			?>
            <a href="<?php    echo $prevlink?>" rel="twipsy" title="previous image" class="btn<?php   if($prevDisabled){?> disabled<?php   }?> pull-left"><span class="icon leftarrow"></span><?php   echo t('Prev')?></a>
            
            <a class="zoomImage btn pull-left" href="#zoomImage" id="zoomimg" rel="twipsy" title="zoom image"><?php   echo t('Zoom')?><span class="icon magnifier"></span></a>
            <?php   if(Config::get(GBX_ALLOW_NOTES)==1){?><a href="#" id="addnotelink" rel="twipsy" title="add a note" class="btn pull-left"><?php   echo t('Add note')?><span class="icon pen"></span></a><?php   }?>
            
			<?php   if(Config::get(GBX_ALLOW_EMBED)==1){?><a href="#" data-controls-modal="embed-modal" rel="twipsy" title="get HTML embed code" data-backdrop="true" class="btn pull-left"><?php   echo t('Embed')?><span class="icon cog"></span></a><?php   }?>
            
      		<?php   if(Config::get(GBX_ALLOW_DOWNLOAD)==1){?><a href="<?php   echo $dUrl?>" rel="twipsy" title="download full size" class="btn pull-left"><?php   echo t('Download')?><span class="icon downarrow"></span></a><?php   }?>

            <a href="<?php   echo $nextlink?>" id="previmg" rel="twipsy" title="next image" class="btn pull-right<?php   if($nextDisabled){?> disabled<?php   }?>"><?php   echo t('Next')?><span class="icon rightarrow"></span></a>
            </div>
                    ...
                    <h4><?php   echo t('Comments and Favorites')?></h4>
                    ...
                    ?>
						...
						<span class="date">(<?php   echo $date->timeSince(strtotime($comments[$i]['entryDate']))?> <?php   echo t('ago')?>)</span>
                        <?php  
						if($editor){
						?>
                    ...
                    <div class="row gbx-comment-form">
                        ...
                        <textarea name="imgComment" id="imgComment" /><?php   echo t('add your comment here...')?></textarea>
                        <div class="clearfix">
                    ...
                    <?php    }else{?>
                    <div class="gbx-comment-form"><p class="gray"><?php   echo t('You must')?> <a href="<?php    echo $this->url('/login')?>"><?php   echo t('login')?></a> <?php   echo t('to post comments and add favorites')?></p></div>
                 ...
                 <?php    if($commenter->isRegistered()){?>
                    ...
                    <?php    }else{?>
                    <div id="noteform" ><p class="gray"><?php   echo t('You must')?> <a href="<?php    echo $this->url('/login')?>"><?php   echo t('login')?></a> <?php   echo t('to write notes')?><div class="form-button"><input type="button" value="Cancel" id="cancelnote" class="btn pull-right pad-right"/></div></p></div>
                    <?php    }?>
                    <?php    }?>
        ...
        <div id="delete-note-modal" class="modal hide" style="display: none; ">
            <div class="modal-header">
              ...
              <h3><?php   echo t('Delete Note')?></h3>
            </div>
            
            <div class="modal-body">
            <form id="CommentDeleteForm" method="post" action="<?php    echo $this->action('delete_note', $imgID)?>">
              <p><?php   echo t('Are you sure that you want to delete this image note?')?>
              <span class="help-block"><?php   echo t('This action cannot be undone.')?></span>
              </p>
              <input name="noteID" id="noteID" type="hidden" value=""/>
            </div>
            <div class="modal-footer">
              <input type="submit" value="Delete" class="btn danger"/>
              <a href="#" class="btn secondary close-this"><?php   echo t('Cancel')?></a>
              </form>
            </div>
          </div>
          ...
          <div id="delete-comment-modal" class="modal hide" style="display: none; ">
            <div class="modal-header">
              <a href="#" class="close">×</a>
              <h3><?php   echo t('Delete Comment')?></h3>
            </div>
            <div class="modal-body">
             <form id="CommentDeleteForm" method="post" action="<?php    echo $this->action('delete_comment', $imgID)?>">
              <p><?php   echo t('Are you sure that you want to delete this comment?')?>
              <span class="help-block"><?php   echo t('This action cannot be undone.')?></span>
              </p>
